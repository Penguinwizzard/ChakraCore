...
GlobOpt::MarkArgumentsUsedForBranch(IR::Instr * instr)
{
    ...
    if (instr->IsBranchInstr() && !instr->AsBranchInstr()->IsUnconditional())
    {
        ...
        // We need to check for (0===arg) and (arg===0), since otherwise we get terrible perf after an inliner changes
        // all instances of one to the other.
...
GlobOpt::OptInstr(IR::Instr *&instr, bool* isInstrRemoved)
{
    ...
    if (instr->GetSrc1() && instr->GetSrc1()->GetKind() == IR::OpndKindSym && instr->m_opcode == Js::OpCode::ProfiledLdFld || instr->m_opcode == Js::OpCode::LdFld || instr->m_opcode == Js::OpCode::ScopedLdFld) {
        ...
        if (sym->IsPropertySym()) {
            ...
            if (this->lengthEquivBv != nullptr && this->lengthEquivBv->Test(originalPropertySym->m_id)) {
                   ...
                   )
                {
                    ...
                    instr->TransferDstAttributesTo(newinstr);
                    newinstr->UnlinkSrc1();
