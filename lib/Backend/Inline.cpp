...
void 
Inline::Optimize(Func *func, IR::Instr *callerArgOuts[], Js::ArgSlot callerArgOutCount, uint recursiveInlineDepth)
{
    ...
    StackSym *symThis = null;
    lastStatementBoundary = nullptr;
    IR::LabelInstr* loopTop = null;
    ...
    FOREACH_INSTR_EDITING(instr, instrNext, func->m_headInstr)
    {
        ...
        if (doInline)
        {
            // TODO: Use the InlineCallInstr opcode attribute?
            ...
            case Js::OpCode::Label:
                {
                    ...
                    {
                        //We only need to know how if we are inside loop or not, it doesn't matter how many nested levels we are in.
            ...
            case Js::OpCode::CallI:
                {
                    ...
                    {                        
                        ...
                        // If the inlinee info is th array constructor, just change the opcode to NewScObjArray
            ...
            case Js::OpCode::CallI:
                {
                    ...
                    bool safeThis = false;
                    if (TryOptimizeCallInstrWithFixedMethod(instr, null, isPolymorphic /*isPolymorphic*/, isBuiltIn /*isBuiltIn*/, isCtor /*isCtor*/, false /*isInlined*/, safeThis /*unused here*/))
                    {
                        Assert(originalCallTargetStackSym != null);
                        ...
                        IR::ByteCodeUsesInstr * useCallTargetInstr = IR::ByteCodeUsesInstr::New(instr->m_func);
                        useCallTargetInstr->SetByteCodeOffset(instr);
                        useCallTargetInstr->Set(originalCallTargetStackSym->m_id);
...
{
    Assert(cachedFixedInlineeCount <= inlineesDataArrayLength);
    __analysis_assume(cachedFixedInlineeCount <= inlineesDataArrayLength);

    Js::FunctionBody* inlineeFuncBody = null;
    while (inlineeJitTimeData)
    {
    ...
    *pIsInlined = true;
    ...
    for(uint i=0; i < cachedFixedInlineeCount; i++)
    {
        ...
        while (fixedFunctionInfoArray[i].nextHasSameFixedField)
        {
            ...
            this->topFunc->PinTypeRef(methodPropertyOpnd->GetType(i)); // Keep the types alive as the types may not be equivalent and, hence, wont be kept alive by EquivalentTypeCache
            i++;
        }
    ...
    const InlineeData& inlineeData, IR::LabelInstr* doneLabel, const StackSym* symCallerThis, bool fixedFunctionSafeThis, uint recursiveInlineDepth)
{
    ...
    if (isInlined)
    {
        ...
        inlinee->m_tailInstr->InsertBefore(doneInstr); //JMP to done at the end
...
Inline::BuildInlinee(Js::FunctionBody* funcBody, const InlineeData& inlineeData, Js::RegSlot returnRegSlot, IR::Instr *callInstr, uint recursiveInlineDepth)
{
    ...
    Func *inlinee = JitAnew(this->topFunc->m_alloc,
                            ...
                            entryPointPolymorphicInlineCacheInfo ? entryPointPolymorphicInlineCacheInfo->GetInlineeInfo(funcBody) : null,
...
#define TRACE_FIXED_FIELDS 1
    ...
    if (printFixedFieldsTrace)
    {
        ...
        Js::FunctionBody* calleeFunctionBody = functionInfo != null && functionInfo->HasBody() ? functionInfo->GetFunctionBody() : null;
        const wchar_t* calleeName = calleeFunctionBody != null ? calleeFunctionBody->GetDisplayName() : L"<unknown>";
    ...
    if (!methodValueSym->IsSingleDef())
    {
        ...
        if (printFixedFieldsTrace)
        {
            ...
            Output::Print(L"FixedFields: function %s (%s): %s non-fixed method <unknown> (%s %s), because callee is not single def.\n", 
                ...
                functionInfo != null ? L"inlining" : L"calling", calleeName, 
                calleeFunctionBody ? calleeFunctionBody->GetDebugNumberSet(debugStringBuffer2) : L"(null)");
            Output::Flush();
        }
#endif
    ...
    IR::Instr* ldMethodFldInstr = methodValueSym->GetInstrDef();
    // We can only optimize calls whose targets come from ScopedLdMethodFld\LdRootMethodFld\LdMethodFld\LdRootFld\LdFld.
       ...
       )
    {
        ...
        if (printFixedFieldsTrace)
        {
            ...
            Output::Print(L"FixedFields: function %s (#%u): %s non-fixed method %s (%s #%u) (cache id: %d), because %s fixed %s %s is disabled.\n",
                callerFunctionBody->GetDisplayName(), callerFunctionBody->GetDebugNumberSet(debugStringBuffer),
                functionInfo != null ? L"inlining" : L"calling", methodPropertyRecord->GetBuffer(), calleeName, 
                calleeFunctionBody ? calleeFunctionBody->GetDebugNumberSet(debugStringBuffer2) : L"(null)",
    ...
    Js::JavascriptFunction const * functionObject = null;
    if (!isPolymorphic)
    {
        functionObject = methodPropertyOpnd->HasFixedValue() ? methodPropertyOpnd->GetFieldValueAsFixedFunction() : null;
    }
    else if (isPolymorphic && isInlined)
    {
        functionObject = methodPropertyOpnd->HasFixedValue() ? methodPropertyOpnd->GetFieldValueAsFixedFunction(i) : null;
    }
    ...
    if (functionInfo == null && Func::IsBuiltInInlinedInLowerer(callInstr->GetSrc1()))
    {
#if TRACE_FIXED_FIELDS
        if (printFixedFieldsTrace)
        {
    ...
    if (functionInfo != null && functionObject->GetFunctionInfo() != functionInfo)
    {
#if TRACE_FIXED_FIELDS
        if (printFixedFieldsTrace)
        {
            ...
            Js::DynamicObject* protoObject = isProto ? methodPropertyOpnd->GetProtoObject() : null;
            ...
            const wchar_t* profileFunctionName    = profileFunctionBody != null ? profileFunctionBody->GetDisplayName() : L"<unknown>";
            ...
            if (PHASE_TRACE(Js::FixedMethodsPhase, callInstr->m_func->GetJnFunction()))
            {
                Output::Print(L"FixedFields: function %s (#%s): function body mismatch for inlinee: %s (%s) 0x%p->0x%p != %s (%s) 0x%p (cache id: %d, layout: %s, type: 0x%p, proto: 0x%p, proto type: 0x%p).\n",
                    ...
                    methodPropertyOpnd->GetType(), protoObject, protoObject != null ? protoObject->GetType() : null);
            }
...
#endif
        // It appears that under certain bailout and re-JIT conditions we may end up with and updated
    ...
    else
    {
        ...
        if (printFixedFieldsTrace)
        {
            ...
            Js::PropertyId methodPropertyId = callerFunctionBody->GetPropertyIdFromCacheId(methodPropertyOpnd->m_inlineCacheIndex);
            Js::PropertyRecord const * const methodPropertyRecord = callerFunctionBody->GetScriptContext()->GetPropertyNameLocked(methodPropertyId);
            const wchar_t* fixedFunctionName = calleeFunctionBody != null ? calleeFunctionBody->GetDisplayName() : L"<unknown>";
            Js::FunctionBody* fixedFunctionBody = functionObject->GetFunctionInfo()->GetFunctionBody();
            const wchar_t* fixedFunctionNumbers = fixedFunctionBody ? fixedFunctionBody->GetDebugNumberSet(debugStringBuffer2) : L"(null)";

            Output::Print(L"FixedFields: function %s (%s): %s fixed method %s (%s %s) (cache id: %d, layout: %s).\n",
                callerFunctionBody->GetDisplayName(), callerFunctionBody->GetDebugNumberSet(debugStringBuffer),
                functionInfo != null ? L"inlining" : L"calling",
...
#undef TRACE_FIXED_FIELDS
    ...
    if (isCtor)
    {
        ...
        if (constructorCache != null && callInstr->IsProfiledInstr())
        {
            ...
            if (PHASE_TRACE(Js::FixedNewObjPhase, callInstr->m_func->GetJnFunction()) || PHASE_TESTTRACE(Js::FixedNewObjPhase, callInstr->m_func->GetJnFunction()))
            {
                ...
                Output::Print(L"FixedNewObj: function %s (%s): fixed new object for %s with %s ctor %s (%s %s)%s\n",
                    ...
                    functionInfo != null ? L"inlined" : L"called",
                    methodPropertyRecord->GetBuffer(), fixedFunctionName, fixedFunctionNumbers,
        ...
        else
        {
            ...
            if (PHASE_TRACE(Js::FixedNewObjPhase, callInstr->m_func->GetJnFunction()) || PHASE_TESTTRACE(Js::FixedNewObjPhase, callInstr->m_func->GetJnFunction()))
            {
                ...
                Output::Print(L"FixedNewObj: function %s (%s): non-fixed new object for %s with %s ctor %s (%s %s), because %s.\n",
                    ...
                    functionInfo != null ? L"inlined" : L"called",
                    methodPropertyRecord->GetBuffer(), fixedFunctionName, fixedFunctionNumbers,
                    constructorCache == null ? L"constructor cache hasn't been cloned" : L"instruction isn't profiled");
                Output::Flush();
            }
#endif
        }
...
Inline::InlineBuiltInFunction(IR::Instr *callInstr, Js::FunctionInfo *funcInfo, Js::OpCode inlineCallOpCode, const Js::FunctionCodeGenJitTimeData* inlinerData, const StackSym *symCallerThis, bool* pIsInlined, uint profileId, uint recursiveInlineDepth)
{
    ...
    *pIsInlined = false;
    ...
    if(inlineCallOpCode == Js::OpCode::InlineMathFloor || inlineCallOpCode == Js::OpCode::InlineMathCeil || inlineCallOpCode == Js::OpCode::InlineMathRound)
    {
        ...
        // TODO: Might still want to inline if SSE4.1 is not available to call the helper directly instead of lowering to ROUNDSD
    ...
    // TODO: should we care about lossy int type spec/benefit from it when aggressive is off?
    ...
    if (!(builtInFlags & Js::BuiltInFlags::BIF_IgnoreDst) && callInstr->GetDst() == null && inlineCallOpCode != Js::OpCode::InlineArrayPop)
    {
    ...
    IR::Instr *inlineBuiltInEndInstr = null;
    ...
    *pIsInlined = true;
    ...
    if(Js::JavascriptLibrary::IsTypeSpecRequired(builtInFlags))
    {
        ...
        useCallTargetInstr->SetByteCodeOffset(callInstr->GetPrevRealInstrOrLabel());
        useCallTargetInstr->Set(originalCallTargetStackSym->m_id);
        ...
        // Mark the StartCall's dst as an inlined arg slot as well so we know this is a inlined start call
...
IR::Instr* Inline::InlineApply(IR::Instr *callInstr, Js::FunctionInfo *funcInfo, const Js::FunctionCodeGenJitTimeData* inlinerData, const StackSym *symCallerThis, bool* pIsInlined, uint callSiteId, uint recursiveInlineDepth)
{
    ...
    *pIsInlined = false;
    ...
    IR::SymOpnd* argumentsLinkOpnd = callInstr->GetSrc2()->AsSymOpnd();
    StackSym *sym = argumentsLinkOpnd->AsSymOpnd()->m_sym->AsStackSym();
    ...
    //Don't inline if this function has formals
    //Don't inline if the arguments object is not singledef
    if (func->GetJnFunction()->GetInParamsCount() != 1
        || !(src1->IsRegOpnd() && src1->AsRegOpnd()->m_sym->IsSingleDef() 
        && (src1->AsRegOpnd()->m_sym->m_instrDef->m_opcode == Js::OpCode::LdHeapArguments 
            || src1->AsRegOpnd()->m_sym->m_instrDef->m_opcode == Js::OpCode::LdLetHeapArguments)))
    {
#if ENABLE_DEBUG_CONFIG_OPTIONS
        wchar_t debugStringBuffer[MAX_FUNCTION_BODY_DEBUG_STRING_SIZE];
#endif
        INLINE_TESTTRACE(L"INLINING: Skip Inline: apply doesn't have .apply(this,arguments) pattern \tCaller: %s (%s)\n",
            inlinerData->GetFunctionBody()->GetDisplayName(), inlinerData->GetFunctionBody()->GetDebugNumberSet(debugStringBuffer));
        //As of now we support only func.apply(this, arguments) pattern.
        return callInstr;
    }
    ...
    *pIsInlined = true;
    ...
    {
        if (InlineApplyTarget(callInstr, inlinerData, funcInfo, symCallerThis, &returnInstr, recursiveInlineDepth))
        {
            Assert(returnInstr);
            return returnInstr;
        }
    ...
    argumentsLinkOpnd->m_sym->AsStackSym()->m_isInlinedArgSlot = true;
    argumentsLinkOpnd->m_sym->AsStackSym()->m_allocated = true;

    IR::RegOpnd* argumentsObj = IR::RegOpnd::New(TyVar, argInstr->m_func);
    IR::Instr *assignInstr = IR::Instr::New(Js::OpCode::LdArgumentsFromStack,  argumentsObj, ldHeapArguments->GetDst(), argInstr->m_func);
    assignInstr->SetByteCodeOffset(argInstr);
    argInstr->InsertBefore(assignInstr);

    // Convert ArgOut to Js::OpCode::ArgOut_A_InlineBuiltIn
    argInstr->ReplaceSrc1(ldHeapArguments->GetDst());
    argInstr->m_opcode = Js::OpCode::ArgOut_A_InlineBuiltIn;
    argInstr->GenerateBytecodeArgOutCapture();

    IR::SymOpnd* linkOpnd = argInstr->GetSrc2()->AsSymOpnd();
    linkOpnd->m_sym->AsStackSym()->m_isInlinedArgSlot = true;
    linkOpnd->m_sym->AsStackSym()->m_allocated = true;
    argInstr = GetDefInstr(linkOpnd);                           //explicit this instruction
    argInstr->m_opcode = Js::OpCode::ArgOut_A_InlineBuiltIn;
    argInstr->GenerateBytecodeArgOutCapture();

    IR::Opnd* thisOpnd = argInstr->GetSrc1();
    linkOpnd = argInstr->GetSrc2()->AsSymOpnd();
    linkOpnd->m_sym->AsStackSym()->m_isInlinedArgSlot = true;
    linkOpnd->m_sym->AsStackSym()->m_allocated = true;

    IR::Instr* argImplicitInstr = GetDefInstr(linkOpnd);        //implicit this instruction.
    argImplicitInstr->m_opcode = Js::OpCode::ArgOut_A_InlineBuiltIn;
    argImplicitInstr->GenerateBytecodeArgOutCapture();
...
//                    CallI_Dynamic     s6.var,  linkOpnd1
    ...
    {
        Assert(callInstr->m_opcode == Js::OpCode::CallIFixed);
        bailOutOnNotStackArgs = IR::BailOutInstr::New(Js::OpCode::BailOnNotStackArgs, IR::BailOutOnInlineFunction,
    ...
    else
    {
        ...
        bailOutOnNotStackArgs = IR::BailOutInstr::New(Js::OpCode::BailOnNotStackArgs, IR::BailOutOnInlineFunction,
            primaryBailoutInstr->GetBailOutInfo(), callInstr->m_func);
    ...
    builtInStartInstr->IterateArgInstrs([&] (IR::Instr* argInstr){
        argInstr->Unlink();
        argInsertInstr->InsertBefore(argInstr);
        argInsertInstr = argInstr;
        return false;
    });
    ...
    startCall->SetSrc1(IR::IntConstOpnd::New(2, TyInt32, func)); //2 args (this pointer & ArgOut_A_From_StackArgs for this direct call to init

    callInstr->InsertBefore(startCall);

    StackSym *symDst = callInstr->m_func->m_symTable->GetArgSlotSym((uint16)(2));
    IR::SymOpnd* linkOpnd1 = IR::SymOpnd::New(symDst, 0, TyMachPtr, func);

    symDst = callInstr->m_func->m_symTable->GetArgSlotSym((uint16)(1));
    IR::Opnd *linkOpnd2 = IR::SymOpnd::New(symDst, 0, TyMachPtr, func);

    //This keeps the stack args alive for bailout to recover
    IR::Instr* argout = IR::Instr::New(Js::OpCode::ArgOut_A_FromStackArgs, linkOpnd1, ldHeapArguments->GetDst(), startCall->GetDst(), func);
    ...
    argout = IR::Instr::New(Js::OpCode::ArgOut_A_Dynamic, linkOpnd2, thisOpnd, linkOpnd1, func);//push explicit this as this pointer
    callInstr->InsertBefore(argout);
    return callInstr;
}

bool Inline::InlineApplyTarget(IR::Instr *callInstr, const Js::FunctionCodeGenJitTimeData* inlinerData, Js::FunctionInfo *applyFuncInfo, const StackSym *symCallerThis, IR::Instr ** returnInstr, uint recursiveInlineDepth)
{
#if ENABLE_DEBUG_CONFIG_OPTIONS
    wchar_t debugStringBuffer[MAX_FUNCTION_BODY_DEBUG_STRING_SIZE];
    wchar_t debugStringBuffer2[MAX_FUNCTION_BODY_DEBUG_STRING_SIZE];
    ...
    IR::Instr* implicitThisArgOut = null;
    IR::Instr* explicitThisArgOut = null;
    IR::Instr* argumentsObjArgOut = null;
    callInstr->IterateArgInstrs([&](IR::Instr* argInstr)
    {
...
Inline::InlineCall(IR::Instr *callInstr, Js::FunctionInfo *funcInfo, const Js::FunctionCodeGenJitTimeData* inlinerData, const StackSym *symCallerThis, bool* pIsInlined, uint callSiteId, uint recursiveInlineDepth)
{
    ...
    *pIsInlined = true;
    ...
    StackSym* originalCallTargetStackSym = callInstr->GetSrc1()->GetStackSym();
    bool safeThis = false;
    if (!TryOptimizeCallInstrWithFixedMethod(callInstr, funcInfo, false /*isPolymorphic*/, true /*isBuiltIn*/, false /*isConstructorInlining*/, true /*isInlined*/, safeThis))
    {
    ...
    IR::Instr* clonedArgout = null;
    IR::Instr* orgArgout = null;
                                              ...
                                              IR::Instr* builtInLdInstr, IR::Instr* targetLdInstr, bool& safeThis, bool isApplyTarget)
{
    ...
    if (TryOptimizeCallInstrWithFixedMethod(callInstr, builtInFuncInfo/*funcinfo for call*/, false /*isPolymorphic*/, false /*isBuiltIn*/, false /*isConstructorInlining*/, true /*isInlined*/, 
        safeThis /*unused here*/, true /*dontOptimizeJustCheck*/))
    {
        ...
        if (!TryOptimizeCallInstrWithFixedMethod(callInstr, targetFunctionInfo, false /*isPolymorphic*/, false /*isBuiltIn*/, false /*isConstructorInlining*/, true /*isInlined*/, 
            safeThis /*unused here*/, true /*dontOptimizeJustCheck*/))
        {
    ...
    safeThis = false;
    TryOptimizeCallInstrWithFixedMethod(callInstr, builtInFuncInfo/*funcinfo for apply/call */, false /*isPolymorphic*/, false /*isBuiltIn*/, false /*isConstructorInlining*/, true /*isInlined*/, safeThis /*unused here*/);
    ...
    safeThis = false;
    TryOptimizeCallInstrWithFixedMethod(callInstr, targetFunctionInfo, false /*isPolymorphic*/, false /*isBuiltIn*/, false /*isConstructorInlining*/, true /*isInlined*/, safeThis /*unused here*/);
...
IR::Instr * Inline::InlineDOMGetterSetterFunction(IR::Instr *ldFldInstr, const Js::FunctionCodeGenJitTimeData *const inlineeData, const Js::FunctionCodeGenJitTimeData *const inlinerData)
{
    ...
    // TODO: make LdMethodFromFlags hoistable. we need to mark the OpCode as OpCanCSE, update the copyprop check for the OpCode, update the
    // cloning of Opcode after hoisting extra.
...
Inline::InlineScriptFunction(IR::Instr *callInstr, const Js::FunctionCodeGenJitTimeData *const inlineeData, const StackSym *symCallerThis, const Js::ProfileId profileId, bool* pIsInlined, uint recursiveInlineDepth)
{
    ...
    *pIsInlined = true;
    ...
    if (callInstr->m_opcode == Js::OpCode::NewScObject || callInstr->m_opcode == Js::OpCode::NewScObjArray)
    {
        ...
        Assert(split && inlineBailoutChecksBeforeInstr != null);
    }
    else
    {
        isFixed = TryOptimizeCallInstrWithFixedMethod(callInstr, inlineeData->GetFunctionInfo(),
    ...
    IR::Instr** callCtorInstrOut) const
{
    ...
    IR::RegOpnd* createObjDst = null;
    IR::Instr* createObjInstr = null;
    ...
    if (newObjInstr->IsProfiledInstr())
    {
        ...
        returnCreatedObject = constructorCache != null && constructorCache->ctorHasNoExplicitReturnValue;
        skipNewScObj = constructorCache != null && constructorCache->skipNewScObject;
        if (!skipNewScObj)
        {
    ...
    // TODO (ObjTypeSpecNewObj): Alternatively, we could split these also, and check for them when lowering CallI.  Also, if we plan
    // to bail out on constructor mismatch, we could set a definitive value type for the results of these constructors, either 
    // here or in glob opt.
    if (skipNewScObj)
    {
    ...
    if (createObjInstrOut != null)
    {
        *createObjInstrOut = createObjInstr;
    }

    if (callCtorInstrOut != null)
    {
...
bool Inline::InlConstFoldArg(IR::Instr *instr, IR::Instr *callerArgOuts[], Js::ArgSlot callerArgOutCount)
{
...
bool Inline::InlConstFold(IR::Instr *instr, IntConstType *pValue, IR::Instr *callerArgOuts[], Js::ArgSlot callerArgOutCount)
{
    IR::Opnd *src1 = instr->GetSrc1();
    ...
    case IR::OpndKindSym:
        if (callerArgOuts && instr->m_opcode == Js::OpCode::ArgIn_A)
        {
            ...
            if (paramSlot <= callerArgOutCount)
            {
                ...
                IR::Instr *instrDef = sym->GetInstrDef();
                // Walk the caller
                return InlConstFold(instrDef, pValue, null, (Js::ArgSlot) - 1);
            }
        }
...
{
    Assert(formalCount <= maxParamCount);
    __analysis_assume(formalCount <= maxParamCount);
...
Inline::SetupInlineeFrame(Func *inlinee, IR::Instr *inlineeStart, Js::ArgSlot actualCount, IR::Opnd *functionObject)
{
    ...
    for (unsigned instrIndex = 0; instrIndex < Js::Constants::InlineeMetaArgCount; instrIndex++)
    {
        ...
        // TODO: These could simply be ArgOut_A instructions that can be inserted into the linked list.
