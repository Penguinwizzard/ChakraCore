...
IRBuilder::BuildReg1(Js::OpCode newOpcode, uint32 offset, Js::RegSlot R0)
{
    ...
    {
        if (this->m_func->GetJnFunction()->NeedScopeObjectForArguments(m_func->GetHasNonSimpleParams()))
        {
...
void IRBuilder::BuildInitCachedScope(int auxOffset, int offset)
{
    ...
    src2Opnd = IR::AddrOpnd::New(this->m_functionBody->GetFormalsPropIdArray(), IR::AddrOpndKindDynamicMisc, m_func);
                            ...
                            Js::ArgSlot argCount, Js::ProfileId profileId, Js::InlineCacheIndex inlineCacheIndex)
{
        ...
        || opcode == Js::OpCode::ProfiledNewScObjectSpread)
    {
        ...
        else
        {
            ...
            ValueType knownReturnType = this->m_func->GetProfileInfo()->GetReturnType(this->m_func->GetJnFunction(), opcode, profileId);
            if (knownReturnType != ValueType::GetObject(ObjectType::UninitializedObject))
            {
