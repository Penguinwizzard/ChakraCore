...
BackwardPass::BackwardPass(Func * func, GlobOpt * globOpt, Js::Phase tag)
    : func(func), globOpt(globOpt), tag(tag), currentPrePassLoop(null), tempAlloc(null),
    preOpBailOutInstrToProcess(null),
...
BackwardPass::DoMarkTempObjects() const
{
    ...
    //   this may cause MarkTempObject convert some temps back to non-temp when it sees a 'transfered exposed use'
    //   from a temp to non-temp. That's in general not supported conversion (while non-temp -> temp is fine).
}
...
// static
bool BackwardPass::DoDeadStore(Func* func, StackSym* sym)
{
...
bool BackwardPass::DoTrackCompoundedIntOverflow() const
{
    return
...
bool BackwardPass::DoTrackNon32BitOverflow() const
{
    // enabled only for IA
...
BackwardPass::CleanupBackwardPassInfoInFlowGraph()
{
    ...
    // The backward pass temp arena has already been delete, we can just reset the data

    FOREACH_BLOCK_IN_FUNC_DEAD_OR_ALIVE(block, this->func)
    {
        block->upwardExposedUses = null;
        block->upwardExposedFields = null;
        block->typesNeedingKnownObjectLayout = null;
        block->fieldHoistCandidates = null;
        block->slotDeadStoreCandidates = null;
        block->byteCodeUpwardExposedUsed = null;
#if DBG
        block->byteCodeRestoreSyms = null;
#endif
        block->tempNumberTracker = null;
        block->tempObjectTracker = null;
#if DBG
        block->tempObjectVerifyTracker = null;
#endif
        ...
        block->noImplicitCallJsArrayHeadSegmentSymUses = null;
        block->noImplicitCallArrayLengthSymUses = null;
...
BackwardPass::MergeSuccBlocksInfo(BasicBlock * block)
{
    ...
    TempNumberTracker * tempNumberTracker = null;    
    TempObjectTracker * tempObjectTracker = null;    
#if DBG
    TempObjectVerifyTracker * tempObjectVerifyTracker = null;
#endif
    ...
    BVSparse<JitArenaAllocator> * noImplicitCallJsArrayHeadSegmentSymUses = null;
    BVSparse<JitArenaAllocator> * noImplicitCallArrayLengthSymUses = null;
    BVSparse<JitArenaAllocator> * upwardExposedUses = null;
    BVSparse<JitArenaAllocator> * upwardExposedFields = null;
    BVSparse<JitArenaAllocator> * typesNeedingKnownObjectLayout = null;
    BVSparse<JitArenaAllocator> * fieldHoistCandidates = null;
    BVSparse<JitArenaAllocator> * slotDeadStoreCandidates = null;
    BVSparse<JitArenaAllocator> * byteCodeUpwardExposedUsed = null;
#if DBG
    uint byteCodeLocalsCount = func->GetJnFunction()->GetLocalsCount();
    StackSym ** byteCodeRestoreSyms = null;
#endif
    ...
    {
        tempObjectVerifyTracker = JitAnew(this->tempAlloc, TempObjectVerifyTracker, this->tempAlloc, block->loop != null); 
    }
#endif
    ...
    if (!block->isDead)
    {
        ...
        JitArenaAllocator *upwardExposedArena = null;
        if(!IsCollectionPass())
        {
            ...
            if (this->DoMarkTempNumbers())
            {
                tempNumberTracker = JitAnew(this->tempAlloc, TempNumberTracker, this->tempAlloc,  block->loop != null);                
            }
            if (this->DoMarkTempObjects())
            {
                tempObjectTracker = JitAnew(this->tempAlloc, TempObjectTracker, this->tempAlloc, block->loop != null); 
            }
        ...
        FOREACH_SUCCESSOR_BLOCK(blockSucc, block)
        {
            ...
            Assert((byteCodeUpwardExposedUsed == null) == !this->DoByteCodeUpwardExposedUsed());
            if (byteCodeUpwardExposedUsed && blockSucc->byteCodeUpwardExposedUsed)
            {
                ...
                if (this->tag == Js::DeadStorePhase)
                {
                    ...
                    {
                        if (byteCodeRestoreSyms[i] == null)
                        {
                            byteCodeRestoreSyms[i] = blockSucc->byteCodeRestoreSyms[i];
                        }
                        else
                        {
                            Assert(blockSucc->byteCodeRestoreSyms[i] == null
                                || byteCodeRestoreSyms[i] == blockSucc->byteCodeRestoreSyms[i]);
                        }
                    }
#endif
                    if (deleteData)
                    {
                        ...
                        blockSucc->byteCodeUpwardExposedUsed = null;
                    }
                }
#if DBG
                if (deleteData)
                {
                    ...
                    blockSucc->byteCodeRestoreSyms = null;
                }
#endif
            ...
            {
                Assert(blockSucc->byteCodeUpwardExposedUsed == null);
                Assert(blockSucc->byteCodeRestoreSyms == null);
            }
            ...
            Assert((blockSucc->upwardExposedUses != null) 
                || (blockSucc->isLoopHeader && (this->IsPrePass() || blockSucc->loop->IsDescendentOrSelf(block->loop))));
            Assert((blockSucc->upwardExposedFields != null) 
                || (blockSucc->isLoopHeader && (this->IsPrePass() || blockSucc->loop->IsDescendentOrSelf(block->loop))));
            Assert((blockSucc->typesNeedingKnownObjectLayout != null) 
                || (blockSucc->isLoopHeader && (this->IsPrePass() || blockSucc->loop->IsDescendentOrSelf(block->loop)))
                || this->tag != Js::DeadStorePhase);
            Assert((blockSucc->fieldHoistCandidates != null) 
                || blockSucc->isLoopHeader
                || !this->DoFieldHoistCandidates(blockSucc->loop));
            Assert((blockSucc->slotDeadStoreCandidates != null)
                || (blockSucc->isLoopHeader && (this->IsPrePass() || blockSucc->loop->IsDescendentOrSelf(block->loop))) 
                || !this->DoDeadStoreSlots());
            Assert((blockSucc->tempNumberTracker != null) 
                || (blockSucc->isLoopHeader && (this->IsPrePass() || blockSucc->loop->IsDescendentOrSelf(block->loop)))
                || !this->DoMarkTempNumbers());        
            Assert((blockSucc->tempObjectTracker != null) 
                || (blockSucc->isLoopHeader && (this->IsPrePass() || blockSucc->loop->IsDescendentOrSelf(block->loop)))
                || !this->DoMarkTempObjects());     
            Assert((blockSucc->tempObjectVerifyTracker != null) 
                || (blockSucc->isLoopHeader && (this->IsPrePass() || blockSucc->loop->IsDescendentOrSelf(block->loop)))
                || !this->DoMarkTempObjectVerify());  
            if (blockSucc->upwardExposedUses != null)
            {
                                   ...
                                   || (this->IsPrePass() && blockSucc->backwardPassCurrentLoop == this->currentPrePassLoop)))
                {
                    ...
                    blockSucc->upwardExposedUses = null;
                }
            }

            if (blockSucc->upwardExposedFields != null)
            {
                                   ...
                                   || (this->IsPrePass() && blockSucc->backwardPassCurrentLoop == this->currentPrePassLoop)))
                {
                    ...
                    blockSucc->upwardExposedFields = null;
                }
            }
            ...
            if (fieldHoistCandidates && blockSucc->fieldHoistCandidates != null)
            {
                fieldHoistCandidates->Or(blockSucc->fieldHoistCandidates);
                if (deleteData)
                {
                    JitAdelete(this->tempAlloc, blockSucc->fieldHoistCandidates);
                    blockSucc->fieldHoistCandidates = null;
                }
            }
            if (blockSucc->slotDeadStoreCandidates != null)
            {
                slotDeadStoreCandidates->And(blockSucc->slotDeadStoreCandidates);
                if (deleteData)
                {
                    JitAdelete(this->tempAlloc, blockSucc->slotDeadStoreCandidates);
                    blockSucc->slotDeadStoreCandidates = null;
                }
            }
            if (blockSucc->tempNumberTracker != null)
            {                
                Assert((blockSucc->loop != null) == blockSucc->tempNumberTracker->HasTempTransferDependencies());
                tempNumberTracker->MergeData(blockSucc->tempNumberTracker, deleteData);        
                if (deleteData)
                {
                    blockSucc->tempNumberTracker = null;
                }                    
            }
            if (blockSucc->tempObjectTracker != null)
            {                
                Assert((blockSucc->loop != null) == blockSucc->tempObjectTracker->HasTempTransferDependencies());
                tempObjectTracker->MergeData(blockSucc->tempObjectTracker, deleteData);        
                if (deleteData)
                {
                    blockSucc->tempObjectTracker = null;
                }                    
            }
#if DBG
            if (blockSucc->tempObjectVerifyTracker != null)
            {                
                Assert((blockSucc->loop != null) == blockSucc->tempObjectVerifyTracker->HasTempTransferDependencies());
                tempObjectVerifyTracker->MergeData(blockSucc->tempObjectVerifyTracker, deleteData);        
                if (deleteData)
                {
                    blockSucc->tempObjectVerifyTracker = null;
                }                    
            }
#endif
            ...
            if (blockSucc->stackSymToFinalType != null)
            {
#if DBG_DUMP
                ...
                else if (stackSymToFinalType != null)
                {
                    if (this->IsPrePass())
                    {
                ...
                if (deleteData)
                {
                    ...
                    blockSucc->stackSymToFinalType = null;
                }
            }
            else
            { 
...
#endif
                if (blockSucc->stackSymToWriteGuardsMap != null)
                {
#if DBG_DUMP
                    if (PHASE_VERBOSE_TRACE(Js::TraceObjTypeSpecWriteGuardsPhase, this->func))
                    {
...
#endif
                    if (stackSymToWriteGuardsMap == null)
                    {
                    ...
                    if (deleteData)
                    {
                        ...
                        blockSucc->stackSymToWriteGuardsMap = null;
                    }
                }
                else
                {
...
#endif
                if (blockSucc->stackSymToGuardedProperties != null)
                {
#if DBG_DUMP
                    if (PHASE_VERBOSE_TRACE(Js::TraceObjTypeSpecTypeGuardsPhase, this->func))
                    {
...
#endif
                    if (stackSymToGuardedProperties == null)
                    {
                    ...
                    if (deleteData)
                    {
                        ...
                        blockSucc->stackSymToGuardedProperties = null;
                    }
                }
                else
                {
            ...
            if (blockSucc->noImplicitCallUses != null)
            {
                noImplicitCallUses->Or(blockSucc->noImplicitCallUses);
                if (deleteData)
                {
                    JitAdelete(this->tempAlloc, blockSucc->noImplicitCallUses);
                    blockSucc->noImplicitCallUses = null;
                }
            }
            if (blockSucc->noImplicitCallNoMissingValuesUses != null)
            {
                ...
                if (deleteData)
                {
                    ...
                    blockSucc->noImplicitCallNoMissingValuesUses = null;
                }
            }
            if (blockSucc->noImplicitCallNativeArrayUses != null)
            {
                ...
                if (deleteData)
                {
                    ...
                    blockSucc->noImplicitCallNativeArrayUses = null;
                }
            }
            if (blockSucc->noImplicitCallJsArrayHeadSegmentSymUses != null)
            {
                ...
                if (deleteData)
                {
                    ...
                    blockSucc->noImplicitCallJsArrayHeadSegmentSymUses = null;
                }
            }
            if (blockSucc->noImplicitCallArrayLengthSymUses != null)
            {
                ...
                if (deleteData)
                {
                    ...
                    blockSucc->noImplicitCallArrayLengthSymUses = null;
                }
            }
    ...
    if (tempObjectVerifyTracker)
    {
        FOREACH_DEAD_SUCCESSOR_BLOCK(deadBlockSucc, block)
        {
            ...
            if (deadBlockSucc->tempObjectVerifyTracker != null)
            {                
                Assert((deadBlockSucc->loop != null) == deadBlockSucc->tempObjectVerifyTracker->HasTempTransferDependencies());
            ...
            if (deadBlockSucc->byteCodeUpwardExposedUsed)
            {
                ...
                if (this->tag == Js::DeadStorePhase)
                {
                    ...
                    for (uint i = 0; i < byteCodeLocalsCount; i++)
                    {
                        ...
                        {
                            Assert(deadBlockSucc->byteCodeRestoreSyms[i] == null
                                || byteCodeRestoreSyms[i] == deadBlockSucc->byteCodeRestoreSyms[i]);
                        }
                    }
#endif
    ...
    else
    {
        ...
        {
            Assert(block->slotDeadStoreCandidates == null);
            Assert(block->tempNumberTracker == null);
            Assert(block->tempObjectTracker == null);
            Assert(block->tempObjectVerifyTracker == null);
            Assert(block->upwardExposedUses == null);
            Assert(block->upwardExposedFields == null);
            Assert(block->typesNeedingKnownObjectLayout == null);
            Assert(block->fieldHoistCandidates == null);
            // byteCodeUpwardExposedUsed is required to populate the writeThroughSymbolsSet for the try region in the backwards pass
            Assert(block->byteCodeUpwardExposedUsed == null || (this->tag == Js::BackwardPhase && this->func->HasTry() && this->func->DoOptimizeTryCatch())); 
            Assert(block->byteCodeRestoreSyms == null);
            ...
            Assert(block->noImplicitCallJsArrayHeadSegmentSymUses == null);
            Assert(block->noImplicitCallArrayLengthSymUses == null);
        }
        else
        {
...
BackwardPass::DeleteBlockData(BasicBlock * block)
{
    ...
    if (block->upwardExposedFields != null)
    {
        JitArenaAllocator *upwardExposedArena = (this->tag == Js::BackwardPhase) ? this->globOpt->alloc : this->tempAlloc;
    ...
    if (block->fieldHoistCandidates != null)
    {
        JitAdelete(this->tempAlloc, block->fieldHoistCandidates);
        block->fieldHoistCandidates = null;
    }
    if (block->byteCodeUpwardExposedUsed != null)
    {
...
void BackwardPass::ProcessLoopCollectionPass(BasicBlock *const lastBlock)
{
    ...
    // First pass
    BasicBlock *firstInnerLoopHeader = null;
    {
#if DBG_DUMP
        if(IsTraceEnabled())
        {
        ...
        FOREACH_BLOCK_BACKWARD_IN_RANGE_DEAD_OR_ALIVE(block, lastBlock, null)
        {
            ProcessBlock(block);
...
BackwardPass::ProcessBailOutArgObj(BailOutInfo * bailOutInfo, BVSparse<JitArenaAllocator> * byteCodeUpwardExposedUsed)
{
    ...
    if (this->globOpt->TrackArgumentsObject() && bailOutInfo->capturedValues.argObjSyms)
    {
        FOREACH_BITSET_IN_SPARSEBV(symId, bailOutInfo->capturedValues.argObjSyms)
        {
            ...
            {
                if (bailOutInfo->usedCapturedValues.argObjSyms == null)
                {
...
BackwardPass::ProcessBailOutConstants(BailOutInfo * bailOutInfo, BVSparse<JitArenaAllocator> * byteCodeUpwardExposedUsed, BVSparse<JitArenaAllocator>* bailoutReferencedArgSymsBv)
{
    ...
    // Remove constants that we already going to restore
    ...
    FOREACH_SLISTBASE_ENTRY_EDITING(ConstantStackSymValue, value, &bailOutInfo->capturedValues.constantValues, iter)
    {
        ...
        {
            // Constant don't need to be restore, delete.
...
BackwardPass::ProcessBailOutCopyProps(BailOutInfo * bailOutInfo, BVSparse<JitArenaAllocator> * byteCodeUpwardExposedUsed, BVSparse<JitArenaAllocator>* bailoutReferencedArgSymsBv)
{
    ...
    // Remove copy prop that we already going to restore
    ...
    FOREACH_SLISTBASE_ENTRY_EDITING(CopyPropSyms, copyPropSyms, &bailOutInfo->capturedValues.copyPropSyms, iter)
    {
        ...
        {
            // Copy prop need to be restore, add it to the restore list
            ...
            */
            ...
            if(instr->GetBailOutKind() & IR::BailOutOnResultConditions &&
                ...
                dstSym->IsInt32() &&
                dstSym->IsTypeSpec() &&
                dstSym->GetVarEquivSym(null) == copyPropSyms.Value() &&
                instr->GetSrc1() &&
            ...
            else if (block->upwardExposedUses->Test(stackSym->m_id) && !block->upwardExposedUses->Test(copyPropSyms.Value()->m_id))
            {
                ...
                // No point in extended the lifetime of the copy prop sym unnecessary
            }
            else
            {
            ...
            if (bailOutInfo->liveLosslessInt32Syms->Test(symId))
            {
                ...
                int32StackSym = stackSym->GetInt32EquivSym(null);
            ...
            if (int32StackSym != null)
            {
                Assert(float64StackSym == NULL);
        ...
        {
            // Copy prop don't need to be restore, delete.
            iter.RemoveCurrent(allocator);
        }
    }
    NEXT_SLISTBASE_ENTRY_EDITING;
...
BackwardPass::ProcessBailOutInfo(IR::Instr * instr)
{
    if (this->tag == Js::BackwardPhase)
    {
        ...
        Assert(!instr->HasBailOutInfo() || instr->GetBailOutInfo()->byteCodeUpwardExposedUsed == null || (this->func->HasTry() && this->func->DoOptimizeTryCatch()));
    ...
    if (instr->IsByteCodeUsesInstr())
    {
        ...
        if (this->func->hasBailout)
        {
            ...
            if (dst)
            {
                ...
                {
                    dstStackSym = dstStackSym->GetVarEquivSym(null);
                }

                // If the current region is a Try, symbols in its write-through set shoouldn't be cleared.
                    ...
                    !this->CheckWriteThroughSymInRegion(this->currentRegion, dstStackSym))
                {
                    ...
                    {
                        this->currentBlock->byteCodeRestoreSyms[dstStackSym->GetByteCodeRegSlot()] = null;
                    }
#endif
                }
            }
            ...
            if (byteCodeUpwardExposedUsed != null)
            {
                ...
                FOREACH_BITSET_IN_SPARSEBV(symId, byteCodeUpwardExposedUsed)
                {
                    ...
                    if (stackSym->GetByteCodeFunc() == this->func)
                    {
                        ...
                        {
                            AssertMsg(this->currentBlock->byteCodeRestoreSyms[byteCodeRegSlot] == null,
    ...
    if (instr->HasBailOutInfo())
    {
        ...
        if (bailOutInfo->bailOutInstr == instr)
        {
            if(instr->GetByteCodeOffset() == Js::Constants::NoByteCodeOffset ||
                bailOutInfo->bailOutOffset > instr->GetByteCodeOffset())
            {
                // Currently only have post-op bailout with BailOutOnImplicitCalls
                // or JIT inserted operation (which no byte code offsets)
                // If there are other bailout that we want to bailout after the operation
                // we have to make sure that it still doesn't do the implicit call
                // if it is done on the stack object (otherwise, the stack object
                // will be passed to the implicit call functions
...
BackwardPass::IsImplicitCallBailOutCurrentlyNeeded(IR::Instr * instr, bool mayNeedImplicitCallBailOut, bool hasLiveFields)
{
    return this->globOpt->IsImplicitCallBailOutCurrentlyNeeded(
        instr, NULL, NULL, this->currentBlock, hasLiveFields, mayNeedImplicitCallBailOut, false);
}
...
BackwardPass::DeadStoreTypeCheckBailOut(IR::Instr * instr)
{
    ...
    {
        Assert(instr->GetDst() == null && instr->GetSrc1() != null && instr->GetSrc2() == null);
        instr->m_opcode = Js::OpCode::Nop;
        instr->FreeSrc1();
        instr->ClearBailOutInfo();
        if (this->preOpBailOutInstrToProcess == instr)
        {
            this->preOpBailOutInstrToProcess = null;
        }
        return;
    }

    // TODO (ObjTypeSpec): Consider removing CheckFixedMethodFld without bailout much like we do with CheckObjType.

    // We don't need BailOutFailedTypeCheck but may need BailOutOnImplicitCall.
    // Question: are we in the loop landing pad? If so, no bailout, since implicit calls will be checked at
...
BackwardPass::ProcessBailOutInfo(IR::Instr * instr, BailOutInfo * bailOutInfo)
{
    ...
    We dont have flow edges from all the possible exception points in the try to the catch block. As a result, this bailout should 
    ...
    */
    ...
    Assert(bailOutInfo->byteCodeUpwardExposedUsed == null || (this->func->HasTry() && this->func->DoOptimizeTryCatch()));
    ...
    //      3. Makred as m_isBailOutReferenced = true & added to upwardExposedUsed bit vector to ensure we do not dead store their defs.
    ...
    if (bailOutInfo->liveVarSyms)
    {
        ...
        {
            // Add to byteCodeUpwardExposedUsed the the non-temp local vars used so far to restore during bail out.
        ...
        if (instr->m_opcode != Js::OpCode::BailOnException) // see comment at the beginning of this function
        {
            ...
            FOREACH_BITSET_IN_SPARSEBV(symId, tempBv)
            {
                ...
                StackSym * int32StackSym = stackSym->GetInt32EquivSym(null);
                Assert(int32StackSym);
            ...
            FOREACH_BITSET_IN_SPARSEBV(symId, tempBv)
            {
                StackSym * stackSym = this->func->m_symTable->FindStackSym(symId);
                Assert(stackSym->GetType() == TyVar);
                StackSym * float64StackSym = stackSym->GetFloat64EquivSym(null);
                Assert(float64StackSym);
...
BackwardPass::ProcessBlock(BasicBlock * block)
{
    ...
    FOREACH_INSTR_BACKWARD_IN_BLOCK_EDITING(instr, instrPrev, block)
    {
        ...
        if (instr->m_opcode == Js::OpCode::TryCatch)
        {
            ...
            {
                Assert(instr->m_next->IsLabelInstr() && (instr->m_next->AsLabelInstr()->GetRegion() != null));
    ...
    {
        Assert(block->loop->fieldHoistCandidates == null);
...
#if DBG_DUMP
void BackwardPass::DumpBlockData(BasicBlock * block)
{
    block->DumpHeader();
...
bool BackwardPass::UpdateImplicitCallBailOutKind(IR::Instr *const instr, bool needsBailOutOnImplicitCall)
{
    Assert(instr);
    ...
    if (instrBailOutKind & IR::BailOutMarkTempObject)
    {
        ...
        // Remove the mark temp boject bit, as we don't need tit after the dead store pass        
...
bool BackwardPass::ProcessNoImplicitCallUses(IR::Instr *const instr)
{
    Assert(instr);
...
void BackwardPass::ProcessNoImplicitCallDef(IR::Instr *const instr)
{
    Assert(tag == Js::DeadStorePhase);
    Assert(instr);
...
template<class F>
IR::Opnd *BackwardPass::FindNoImplicitCallUse(
...
void BackwardPass::ProcessArrayRegOpndUse(IR::Instr *const instr, IR::ArrayRegOpnd *const arrayRegOpnd)
{
    Assert(tag == Js::DeadStorePhase);
    Assert(!IsCollectionPass());
    Assert(instr);
        ...
        (!isJsArray || instr->m_opcode != Js::OpCode::NoImplicitCallUses))
    {
        ...
        if(headSegmentIsLoadedButUnused || headSegmentLengthIsLoadedButUnused)
        {
            ...
            IR::ArrayRegOpnd *noImplicitCallArrayUse = null;
            if(isJsArray)
            {
                IR::Opnd *const use =
...
void BackwardPass::ProcessNewScObject(IR::Instr* instr)
{
    ...
    if (instr->HasBailOutInfo())
    {
        ...
        if (block->upwardExposedUses->Test(objSym->m_id))
        {
            ...
            if (block->stackSymToFinalType != nullptr)
            {
                    ...
                    pBucket->GetFinalType() != pBucket->GetInitialType())
                {
                    ...
                    if (pBucket->deadStoreUnavailableFinalType == null)
                    {
                        pBucket->deadStoreUnavailableFinalType = pBucket->GetFinalType();
                    }
                    pBucket->SetInitialType(null);
                    pBucket->SetFinalType(null);
#else
        ...
        else
        {
            ...
            ObjTypeGuardBucket* bucket = block->stackSymToGuardedProperties != null ? 
                block->stackSymToGuardedProperties->Get(objSym->m_id) : null;
            Assert(bucket == null || bucket->GetGuardedPropertyOps()->IsEmpty());
#endif
        }
    }
}

void BackwardPass::UpdateArrayValueTypes(IR::Instr *const instr, IR::Opnd *opnd)
{
    Assert(tag == Js::DeadStorePhase);
    Assert(!IsPrePass());
    Assert(instr);
    ...
    switch(opnd->GetKind())
    {
        ...
        case IR::OpndKindReg:
        {
            ...
            sym = regOpnd->m_sym;
            arrayOpnd = regOpnd->IsArrayRegOpnd() ? regOpnd->AsArrayRegOpnd() : null;
            break;
        }
...
void BackwardPass::UpdateArrayBailOutKind(IR::Instr *const instr)
{
    Assert(!IsPrePass());
    Assert(instr);
    Assert(instr->HasBailOutInfo());
...
BackwardPass::ProcessStackSymUse(StackSym * stackSym, BOOLEAN isNonByteCodeUse)
{
    ...
    if (this->DoByteCodeUpwardExposedUsed())
    {
        if (!isNonByteCodeUse && stackSym->HasByteCodeRegSlot())
        {
            ...
            if (byteCodeUseSym->IsTypeSpec())
            {
                ...
                byteCodeUseSym = byteCodeUseSym->GetVarEquivSym(null);
            }
            ...
            if (byteCodeUseSym->GetByteCodeFunc() == this->func)
            {
                ...
                {
                    AssertMsg(block->byteCodeRestoreSyms[byteCodeRegSlot] == null,
                        "Can't have two active lifetime for the same byte code register");
    ...
    if (this->DoMarkTempObjects())
    {
        Assert((block->loop != null) == block->tempObjectTracker->HasTempTransferDependencies());
        block->tempObjectTracker->ProcessUse(stackSym, this);
    }
#if DBG
    if (this->DoMarkTempObjectVerify())
    {
        Assert((block->loop != null) == block->tempObjectVerifyTracker->HasTempTransferDependencies());
...
BackwardPass::ProcessSymUse(Sym * sym, bool isRegOpndUse, BOOLEAN isNonByteCodeUse)
{
    ...
    if (sym->IsPropertySym())
    {
        ...
        Assert((block->fieldHoistCandidates != null) == this->DoFieldHoistCandidates());
        // TODO (jedmiad): Add support for hoisting CheckFixedFld.  In this case we wouldn't 
        // be hoisting the actual load, but the type check that ensures the fixed function we expect
        // is still ok to use.
        if (block->fieldHoistCandidates && GlobOpt::TransferSrcValue(this->currentInstr))
        {
            // If the instruction doesn't transfer the src value to dst, it will not be copy proped
    ...
    if (!IsCollectionPass() && isRegOpndUse && this->DoMarkTempNumbers())
    {
        ...
        Assert((block->loop != null) == block->tempNumberTracker->HasTempTransferDependencies());        
...
BackwardPass::MayPropertyBeWrittenTo(Js::PropertyId propertyId)
{
    return this->func->anyPropertyMayBeWrittenTo || 
        (this->func->propertiesWrittenTo != null && this->func->propertiesWrittenTo->ContainsKey(propertyId));
}
...
{    
    ...
    // We will not set any bits on the operand and we will ingore them when lowering.
...
BackwardPass::TrackObjTypeSpecProperties(IR::PropertySymOpnd *opnd, BasicBlock *block)
{
    ...
    if (opnd->IsTypeCheckSeqParticipant())
    {
        ...
        FOREACH_BITSET_IN_SPARSEBV(propOpId, guardedPropertyOps)
        {
            ...
            // information for the same property. We simply ingore the conflict and emit an equivalent type check, which
        ...
        // Review (jedmiad): Make sure this drops the guarded operations on the right type check if we have 
        // a transitive assignment of the property owner (q = p, p = o) in the loop.
        if (opnd->NeedsPrimaryTypeCheck())
        {
            ...
            Assert(guardedPropertyOps != null);
...
BackwardPass::TrackObjTypeSpecWriteGuards(IR::PropertySymOpnd *opnd, BasicBlock *block)
{
    ...
    if (this->tag == Js::BackwardPhase)
    {
        ...
        if (opnd->IsMono() && opnd->MayNeedWriteGuardProtection())
        {
            ...
            if (writeGuards == null)
            {
                // The bit bectors we push around the flow graph only need to live as long as this phase.
            ...
            Assert(propertySym->m_writeGuardSym != null);
        ...
        {
            Assert(opnd->GetWriteGuards() == null);
            if (block->stackSymToWriteGuardsMap != null)
            {
...
BackwardPass::TrackAddPropertyTypes(IR::PropertySymOpnd *opnd, BasicBlock *block)
{
    ...
    //// proceed with this optimization.
    //if (PHASE_OFF(Js::ObjTypeSpecStoreWithReadOnlyPropsPhase, this->func) && 
    //    !typeWithoutProperty->GetPrototype()->GetType()->AreThisAndPrototypesEnsuredToHaveOnlyWritableDataProperties())
    //{
    //    return;
    //}
    ...
    Js::Type* finalType = null; 
#if DBG
    Js::Type * deadStoreUnavailableFinalType = null;
#endif
    if (pBucket->GetInitialType() == null || opnd->GetType() != pBucket->GetInitialType())
    {
#if DBG
    ...
    if (!PHASE_OFF(Js::ObjTypeSpecStorePhase, this->func))
    {
        ...
        //      o.x = 
        //  o.y =
        // Where the backward pass will propaty the final type in o.y to o.x, then globopt will copy prop t1 and t2 to o.x.  
        // But not o.y (because of the merge).  then in the dead store pass, o.y's final type will not propagate to t1.x and t2.x 
        // respectively, thus regression the final type.  however, in both case, the type of t1 and t2 are dead anyways.
        //
        // if the type is dead, we don't care if we have regressed the type, as no one is depending on it to skip type check anyways
        if (!opnd->IsTypeDead())
        {
            // This is the type that would have been propagated if we didn't kill it because the type isn't availible     
...
template<class Fn>
void BackwardPass::ForEachAddPropertyCacheBucket(Fn fn)
{
    BasicBlock *block = this->currentBlock;
    if (block->stackSymToFinalType == nullptr)
    {
...
bool BackwardPass::TransitionUndoesObjectHeaderInlining(AddPropertyCacheBucket *data) const
{
    ...
    case IR::OpndKindReg:
        {
            ...
            if(currentInstr->m_opcode == Js::OpCode::BailOnNotArray)
            {
                ...
                if(valueType.IsLikelyArrayOrObjectWithArray())
                {
                    ...
                    // what set of sym is equaivant (copied) from the one we are testing for right now=                    
                    if(valueType.HasNoMissingValues() &&
                        !currentBlock->noImplicitCallNoMissingValuesUses->IsEmpty() &&
                        !IsPrePass())
                    {
...
{
    Assert(!IsCollectionPass());
    // Don't care about type specialied syms
    if (!sym->IsVar())
    {
        return;
    }
...
void BackwardPass::TrackBitWiseOrNumberOp(IR::Instr *const instr)
{
    Assert(instr);
...
BackwardPass::TrackIntUsage(IR::Instr *const instr)
{
    ...
    // MULs with ignoreIntOverflow can still bailout on 53-bit ovf, so they cannot be in the middle of a range
...
void BackwardPass::SetIntOverflowDoesNotMatterIfLastUse(IR::Opnd *const opnd)
{
    StackSym *const stackSym = IR::RegOpnd::TryGetStackSym(opnd);
    if(stackSym && !currentBlock->upwardExposedUses->Test(stackSym->m_id))
    {
...
void BackwardPass::SetIntOverflowMatters(IR::Opnd *const opnd)
{
    StackSym *const stackSym = IR::RegOpnd::TryGetStackSym(opnd);
    if(stackSym)
    {
...
BackwardPass::TrackFloatSymEquivalence(IR::Instr *const instr)
{
    ...
    returned from the funtion, it has to be ensured that s5 is not converted to Var. If the source of the FromVar was null, the
    ...
    if (sym->IsPropertySym())
    {
        ...
        if (this->DoDeadStoreSlots())
        {
            if (propertySym->m_fieldKind == PropertyKindLocalSlots || propertySym->m_fieldKind == PropertyKindSlots)
            {
                ...
                // we should not do any deadslots in asmjs loop body 
    ...
    else
    {
        ...
        if (this->DoByteCodeUpwardExposedUsed())
        {
            if (sym->AsStackSym()->HasByteCodeRegSlot())
            {
                ...
                {
                    // It has to have a var version for byte code regs
                    varSym = varSym->GetVarEquivSym(null);
                }
                ...
                if (!isJITOptimizedReg)
                {
                    ...
                    {
                        // Don't dead the bytecodereg sym, so that we could do write to get the locals inspection
                        if (opnd->IsRegOpnd())
                        {
                    ...
                    if (!keepVarSymLiveForException)
                    {
                        ...
                        {
                            block->byteCodeRestoreSyms[varSym->GetByteCodeRegSlot()] = null;
                        }
#endif
                    }
                }
...
#endif
        PropertySym *unusedPropertySym = NULL;
...
BackwardPass::ProcessTransfers(IR::Instr * instr)
{
        ...
        instr->GetSrc1()->GetStackSym()->HasObjectInfo())
    {
        ...
        while (srcPropertySym != null && !transferFields.IsEmpty())
        {
...
template<typename T>
void BackwardPass::ClearBucketsOnFieldKill(IR::Instr *instr, HashTable<T> *table)
{
    if (table)
    {
...
{
    // The the backward pass, we optimistically will not kill on a[] access
    ...
    case Js::OpCode::BrOnNoProperty:
        // Should not hoist pass these insturctions
        this->currentBlock->fieldHoistCandidates->Clear(instr->GetSrc1()->AsSymOpnd()->m_sym->m_id);
        break;
    }
}

bool BackwardPass::TrackNoImplicitCallInlinees(IR::Instr *instr)
{
    ...
    if (instr->HasBailOutInfo() 
        ...
        || instr->CallsAccessor()
        || GlobOpt::MayNeedBailOnImplicitCall(instr, NULL, NULL) 
...
bool BackwardPass::ProcessInlineeStart(IR::Instr* inlineeStart)
{
...
void BackwardPass::ProcessInlineeEnd(IR::Instr* instr)
{
    if (this->IsPrePass())
    {
        return;
...
bool BackwardPass::ProcessBailOnNoProfile(IR::Instr *instr, BasicBlock *block)
{
    Assert(this->tag == Js::BackwardPhase);
    ...
    FOREACH_PREDECESSOR_BLOCK(pred, block)
    {
        ...
        // ok to hoist this bail to the predecessor.
        ...
        if (hoistBailToPred)
        {
            ...
            if (predInstr->EndsBasicBlock())
            {
                ...
                {
                    // Don't put a bailout in the middle of a switch dispath sequence.
...
bool BackwardPass::ReverseCopyProp(IR::Instr *instr)
{
    // Look for :
    //
    //  t1 = instr
    ...
    IR::ByteCodeUsesInstr *byteCodeUseInstr = NULL;
...
bool BackwardPass::FoldCmBool(IR::Instr *instr)
{
...
void BackwardPass::SetWriteThroughSymbolsSetForRegion(BasicBlock * catchBlock, Region * tryRegion)
{
    ...
    if (this->DoByteCodeUpwardExposedUsed())
    {
        ...
        //                          symbol from the bytecode updward exposed on a def, x should be marked as write-through (as 
...
bool BackwardPass::CheckWriteThroughSymInRegion(Region* region, StackSym* sym)
{
    ...
    // otherwise (current = catch region) look in the first try ancestos's write-through set
...
bool BackwardPass::IsTraceEnabled() const
{
    return
