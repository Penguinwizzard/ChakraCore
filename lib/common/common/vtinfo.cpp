...
VirtualTableRegistry::CreateVtableHashMap(ArenaAllocator * alloc)
{
    ...
    // at class initialization time. Here we add them to our hash table for easy lookup. Note that on a fre build
    // the vtables are merged and thus not all of our types will be registered. So, we can only use this method
    // in a non-fre build. If we wanted use in a fre build, we'll have to explicitly add the vtables to the hash and
