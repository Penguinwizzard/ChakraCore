...
private:
    ...
    {
        return ::Math::PointerCastToIntegral<uint>(address) / L2Count / PageSize;
    }

public:
    static uint GetLevel2Id(void * address)
    {
        return ::Math::PointerCastToIntegral<uint>(address) % (L2Count * PageSize) / PageSize;
    }
...
private:
    ...
    public:
        ...
        {
            uint bitIndex = (::Math::PointerCastToIntegral<uint>(address) % (L2Count * PageSize)) / HeapConstants::ObjectGranularity;
