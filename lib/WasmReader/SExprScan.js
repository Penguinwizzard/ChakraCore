...
if (objArgs.length != 1 && objArgs.length != 2) {
    ...
    file.Close();
    var reg = /WASM_[A-Z0-9\_]*\((wtk[A-Z0-9\_]+)\s*,\s*([a-z0-9]+)\)/g;
    var t = [];
    var s = text.replace(reg, function (a, p1, p2, offset) {
        t.push({ tk: p1, word: p2 });
    });
    ...
    r += "// Generated with the following command line: wscript wasmscan.js " + objArgs(0) + " " + objArgs(1) + "\r\n";
