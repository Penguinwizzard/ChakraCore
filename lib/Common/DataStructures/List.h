...
namespace JsUtil
{
    ...
    private:
        ...
        template <> T * AllocArray<true>(__declspec(guard(overflow)) int size) { return AllocatorNewArrayLeaf(TAllocator, this->alloc, T, size); }
        template <> T * AllocArray<false>(__declspec(guard(overflow)) int size) { return AllocatorNewArray(TAllocator, this->alloc, T, size); }
