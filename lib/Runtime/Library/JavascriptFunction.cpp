...
namespace Js
{
    ...
    Var JavascriptFunction::CallRootFunction(Arguments args, ScriptContext * scriptContext, bool inScript)
    {
        ...
        __try
        {
            ...
            __except (CallRootEventFilter(GetExceptionCode(), GetExceptionInformation()))
            {
                Assert(UNREACHED);
            }
        }
        __finally
        {
            ...
            if (exceptionCode != 0 && exceptionCode != EXCEPTION_BREAKPOINT && exceptionCode != 0xE06D7363)
            {
                // Only C++ exceptions should be handled
