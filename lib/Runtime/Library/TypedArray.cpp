...
namespace Js
{
    ...
    Var TypedArrayBase::CreateNewInstance(Arguments& args, ScriptContext* scriptContext, uint32 elementSize, PFNCreateTypedArray pfnCreateTypedArray)
    {
        ...
        if (args.Info.Count > 1)
        {
            ...
            {
                elementCount = ArrayBuffer::GetToIndex(firstArgument, JSERR_InvalidTypedArrayLength, scriptContext, ArrayBuffer::MaxArrayBufferLength / elementSize);
            }
            else
            {
                if (JavascriptOperators::IsObject(firstArgument))
                {
                    ...
                    else if (JavascriptConversion::ToObject(firstArgument, scriptContext, &jsArraySource))
                    {
                        ...
                        if (!fromExternalObject)
                        {
                            ...
                            elementCount = ArrayBuffer::GetToIndex(lengthVar, JSERR_InvalidTypedArrayLength, scriptContext, ArrayBuffer::MaxArrayBufferLength / elementSize);
                        }
                    }
                }
                else
        ...
        if (arrayBuffer != nullptr)
        {
            ...
            {
                offset = ArrayBuffer::GetToIndex(args[2], JSERR_InvalidTypedArrayLength, scriptContext, byteLength, false);
            ...
            {
                mappedLength = ArrayBuffer::GetToIndex(args[3], JSERR_InvalidTypedArrayLength, scriptContext, ArrayBuffer::MaxArrayBufferLength / elementSize, false);
    ...
    BOOL TypedArrayBase::GetEnumerator(JavascriptStaticEnumerator * enumerator, EnumeratorFlags flags, ScriptContext* requestContext)
    {
    ...
    Var TypedArrayBase::CommonSet(Arguments& args)
    {
        ...
        if (args.Info.Count > 2)
        {
            offset = ArrayBuffer::GetToIndex(args[2], JSERR_InvalidTypedArrayLength, scriptContext, ArrayBuffer::MaxArrayBufferLength, false);
        }
