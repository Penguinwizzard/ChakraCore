...
#pragma prefast(suppress:6236, "DevDiv bug 830883. False positive when PHASE_OFF is #defined as '(false)'.")
    ...
    bool JavascriptOperators::CheckIfTypeIsEquivalent(Type* type, JitEquivalentTypeGuard* guard)
    {
        ...
        if (!isEquivalent)
        {
     	...
     	bool evictCachedType = true;
        if (DynamicType::Is(type->GetTypeId()))
        {
            DynamicType* dynamicType = static_cast<DynamicType*>(type);
            if ((dynamicType->GetTypeHandler()->IsSimpleDictionaryTypeHandler() || dynamicType->GetTypeHandler()->IsDictionaryTypeHandler()) &&
                !dynamicType->GetIsShared())
            {
                //type->GetScriptContext()->GetThreadContext()->evictType++;
                evictCachedType = false;
            }
        }
        ...
        else
        {
            ...
            if(evictCachedType) // here equivTypes[index] is not empty, so only evict the type if evictCachedType is true
            {    
                if (PHASE_TRACE1(Js::EquivObjTypeSpecPhase))
                {
                    Output::Print(_u("Evicted a type from equiv type cache because of a non shared dictionary type"));
                }
                __analysis_assume(index < EQUIVALENT_TYPE_CACHE_SIZE);
