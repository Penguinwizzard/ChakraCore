...
#define IncInlineCacheCount(counter)
        ...
        {
            PHASE_PRINT_TRACE(Js::DepolymorphizePhase, topFunctionBody, L"One hit in %s, caller %s\n", functionBody->GetDisplayName(), topFunctionBody->GetDisplayName());
        ...
        ObjTypeSpecFldInfo* info = RecyclerNew(recycler, ObjTypeSpecFldInfo,
            id, typeId, nullptr, typeSet, usesAuxSlot, isProto, isAccessor, hasFixedValue, hasFixedValue, doesntHaveEquivalence, !thisObjectTypeMatched ? true : false, slotIndex, propertyId,
            prototypeObject, propertyGuard, nullptr, fixedFieldInfoArray, fixedFunctionCount/*, null, null, null*/);
