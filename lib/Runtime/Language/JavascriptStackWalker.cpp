...
namespace Js
{
    ...
    bool JavascriptStackWalker::TryGetByteCodeOffsetFromNativeFrame(uint32& offset) const
    {
        ...
        if (InlinedFramesOnStack() && TryGetByteCodeOffsetOfInlinee(function, loopNum, pCodeAddr, &inlinee, offset, usedInternalFrameInfo))
        {
            return true;
        }
    ...
    bool JavascriptStackWalker::TryGetByteCodeOffsetOfInlinee(Js::JavascriptFunction* parentFunction, uint loopNum, DWORD_PTR pCodeAddr, Js::FunctionBody** inlinee, uint32& offset, bool useInternalFrameInfo) const
    {
        ...
        // of the physical frame. All other inlined frames use the offset of the inlined frame above them
        ...
        *inlinee = InlinedFramesBeingWalked() ? inlinedFrameWalker.GetFunctionObject()->GetFunctionBody() : nullptr;
        ...
        else if (ScriptFunction::Is(parentFunction) && InlinedFramesOnStack()
    ...
    bool JavascriptStackWalker::InlinedFramesOnStack() const
    {
        if (lastInternalFrameInfo.codeAddress)
        {
            return lastInternalFrameInfo.inlinedFramesOnStack;
        }
        ...
        if (this->IsJavascriptFrame())
        {
            ...
            if (this->interpreterFrame)
            {
                ...
                if (this->interpreterFrame->GetFlags() & InterpreterStackFrameFlags_FromBailOut)
                {
                    ...
                    else if (isCurrentPhysicalFrameForLoopBody)
                    {
                        ...
                        SetCachedInternalFrameInfo(InternalFrameType_LoopBody, InternalFrameType_LoopBody, function, false /*inlinedFramesOnStack*/);
                    }
#else
            ...
            else if (!this->isNativeLibraryFrame)
            {
                ...
                if (!InlinedFramesOnStack() && includeInlineFrames)
                {
                    ...
                    {
                        this->inlinedFramesBeingWalked = inlinedFrameWalker.Next(inlinedFrameCallInfo);
                        this->inlinedFramesOnStack = true;
                        Assert(inlinedFramesBeingWalked);
                    }
                }
#endif
    ...
    JavascriptStackWalker::JavascriptStackWalker(ScriptContext * scriptContext, bool useEERContext, PVOID returnAddress, bool _forceFullWalk /*=false*/) :
        ...
        previousInterpreterFrameIsFromBailout(false), previousInterpreterFrameIsForLoopBody(false), inlinedFramesOnStack(false)
    {
        ...
        if (inlinedFramesBeingWalked)
        {
            ...
            if (!inlinedFramesBeingWalked)
            {
                ...
                if ((this->IsCurrentPhysicalFrameForLoopBody()))
                {
                    ...
                    this->SetCachedInternalFrameInfo(InternalFrameType_LoopBody, InternalFrameType_LoopBody, this->GetCurrentFunctionFromPhysicalFrame(), true /*inlinedFramesOnStack*/);
...
//#endif
            ...
            if (((CallInfo const *)&argv[JavascriptFunctionArgIndex_CallInfo])->Flags & CallFlags_InternalFrame)
            {
                ...
                SetCachedInternalFrameInfo(InternalFrameType_LoopBody, InternalFrameType_LoopBody, funcObj, false /*inlinedFramesOnStack*/);
                return false;
            }
    ...
    void JavascriptStackWalker::SetCachedInternalFrameInfo(InternalFrameType frameType, InternalFrameType loopBodyFrameType, JavascriptFunction* function, bool inlinedFramesOnStack)
    {
        if (!this->lastInternalFrameInfo.codeAddress)
        {
            this->lastInternalFrameInfo.Set(this->GetCurrentCodeAddr(), this->currentFrame.GetFrame(), this->currentFrame.GetStackCheckCodeHeight(), frameType, loopBodyFrameType, function, inlinedFramesOnStack);
        }
    ...
    void InternalFrameInfo::Set(void *codeAddress, void *framePointer, size_t stackCheckCodeHeight, InternalFrameType frameType, InternalFrameType loopBodyFrameType, JavascriptFunction* function, bool inlinedFramesOnStack)
    {
