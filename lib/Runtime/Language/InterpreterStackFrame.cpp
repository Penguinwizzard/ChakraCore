...
#define PROCESS_A1U1toXXWitCache_COMMON(name, func, suffix) \
    case OpCode::name: \
    { \
...
#define PROCESS_A1U1toXXWitCache(name, func) PROCESS_A1U1toXXWitCache_COMMON(name, func,)
...
const byte * InterpreterStackFrame::OP_ProfiledLoopBodyStart(const byte * ip)
{
    ...
    ForInObjectEnumerator * InterpreterStackFrame::GetForInEnumerator(uint index)
    {
        Assert(index < this->m_functionBody->GetForInLoopDepth());
        return &this->forInObjectEnumerators[index];
    }

    void InterpreterStackFrame::OP_InitForInEnumerator(Var object, uint forInLoopId)
    {
        JavascriptOperators::OP_InitForInEnumerator(object, GetForInEnumerator(forInLoopId), this->GetScriptContext());
    }

    void InterpreterStackFrame::OP_InitForInEnumeratorWithCache(Var object, uint forInLoopId, ProfileId profileId)
    {
        JavascriptOperators::OP_InitForInEnumerator(object, GetForInEnumerator(forInLoopId), this->GetScriptContext(), 
