...
bool
DynamicProfileInfo::NeedProfileInfoList()
{
...
void
DynamicProfileInfo::Initialize(FunctionBody *const functionBody)
{
...
bool
DynamicProfileInfo::EnableImplicitCallFlags(const FunctionBody *const functionBody)
{
...
JavascriptMethod
DynamicProfileInfo::EnsureDynamicProfileInfo(ScriptFunction * function)
{
...
bool 
DynamicProfileInfo::hasLdFldCallSiteInfo()
{
...
DynamicProfileInfo::RecordLdFldCallSiteInfo(FunctionBody* functionBody, RecyclableObject* callee, bool callApplyTarget)
{
    ...
    else if (functionBody->CheckCalleeContextForInlining(calleeFunctionInfo->GetFunctionProxy())) //If functionInfo !have body, above else if is executed; otherwise it has body and therefore has proxy
    {
        return SetBits();
    }
    return false;
...
void 
DynamicProfileInfo::RecordConstParameterAtCallSite(ProfileId callSiteId, int argNum)
{
...
uint16
DynamicProfileInfo::GetConstantArgInfo(ProfileId callSiteId)
{
    return callSiteInfo[callSiteId].isArgConstant;
}

void
DynamicProfileInfo::RecordCallSiteInfo(FunctionBody* functionBody, ProfileId callSiteId, FunctionInfo* calleeFunctionInfo, JavascriptFunction* calleeFunction, ArgSlot actualArgCount, bool isConstructorCall, InlineCacheIndex ldFldInlineCacheId)
{
#if DBG_DUMP || defined(DYNAMIC_PROFILE_STORAGE) || defined(RUNTIME_DATA_COLLECTION)
    // If we persistsAcrossScriptContext, the dynamic profile info may be refered to by multiple function body from
...
bool
DynamicProfileInfo::IsPolymorphicCallSite(Js::LocalFunctionId curFunctionId, Js::SourceId curSourceId, Js::LocalFunctionId oldFunctionId, Js::SourceId oldSourceId)
{
...
void 
DynamicProfileInfo::CreatePolymorphicDynamicProfileCallSiteInfo(FunctionBody *funcBody, ProfileId callSiteId, Js::LocalFunctionId functionId, Js::LocalFunctionId oldFunctionId, Js::SourceId sourceId, Js::SourceId oldSourceId)
{
...
void 
DynamicProfileInfo::ResetPolymorphicCallSiteInfo(ProfileId callSiteId, Js::LocalFunctionId functionId)
{
...
void
DynamicProfileInfo::SetFunctionIdSlotForNewPolymorphicCall(ProfileId callSiteId, Js::LocalFunctionId curFunctionId, Js::SourceId curSourceId, Js::FunctionBody *inliner)
{
...
bool
DynamicProfileInfo::HasCallSiteInfo(FunctionBody* functionBody)
{
...
bool
DynamicProfileInfo::GetPolymorphicCallSiteInfo(FunctionBody* functionBody, ProfileId callSiteId, bool *isConstructorCall, __inout_ecount(functionBodyArrayLength) FunctionBody** functionBodyArray, uint functionBodyArrayLength)
{
    Assert(functionBody);
    ...
    *isConstructorCall = callSiteInfo[callSiteId].isConstructorCall;
    ...
    if (callSiteInfo[callSiteId].isPolymorphic)
    {
        ...
        for(uint i = 0; i < functionBodyArrayLength; i++)
        {
            ...
            {
                AssertMsg(i >= 2, "We found atleast two function Body");
                return true;
            }
...
bool
DynamicProfileInfo::HasCallSiteInfo(FunctionBody* functionBody, ProfileId callSiteId)
{
    Assert(functionBody);
    const auto callSiteCount = functionBody->GetProfiledCallSiteCount();
    Assert(callSiteId < callSiteCount);
...
FunctionInfo *
DynamicProfileInfo::GetCallSiteInfo(FunctionBody* functionBody, ProfileId callSiteId, bool *isConstructorCall, bool *isPolymorphicCall)
{
    Assert(functionBody);
    const auto callSiteCount = functionBody->GetProfiledCallSiteCount();
    Assert(callSiteId < callSiteCount);
...
uint 
DynamicProfileInfo::GetLdFldCacheIndexFromCallSiteInfo(FunctionBody* functionBody, ProfileId callSiteId)
{
    Assert(functionBody);
    const auto callSiteCount = functionBody->GetProfiledCallSiteCount();
    Assert(callSiteId < callSiteCount);
...
void
DynamicProfileInfo::RecordElementLoad(FunctionBody* functionBody, ProfileId ldElemId, const LdElemInfo& info)
{
...
void
DynamicProfileInfo::RecordElementStore(FunctionBody* functionBody, ProfileId stElemId, const StElemInfo& info)
{
...
ArrayCallSiteInfo * 
DynamicProfileInfo::GetArrayCallSiteInfo(FunctionBody *functionBody, ProfileId index) const
{
...
__inline
void
DynamicProfileInfo::RecordFieldAccess(FunctionBody* functionBody, uint fieldAccessId, Var object, FldInfoFlags flags)
{
...
__inline
void 
DynamicProfileInfo::RecordSwitchType(FunctionBody* body, ProfileId switchId, Var object)
{
...
ValueType 
DynamicProfileInfo::GetSwitchType(FunctionBody* body, ProfileId switchId) const
{
    Assert(switchId < body->GetProfiledSwitchCount());
...
void
DynamicProfileInfo::SetHasNewPolyFieldAccess(FunctionBody *functionBody)
{
...
__inline
void
DynamicProfileInfo::RecordSlotLoad(FunctionBody* functionBody, ProfileId slotLoadId, Var object)
{
...
FldInfoFlags
DynamicProfileInfo::MergeFldInfoFlags(FldInfoFlags oldFlags, FldInfoFlags newFlags)
{
...
__inline
void
DynamicProfileInfo::RecordReturnTypeOnCallSiteInfo(FunctionBody* functionBody, ProfileId callSiteId, Var object)
{
    Assert(callSiteId < functionBody->GetProfiledCallSiteCount());
...
__inline
void
DynamicProfileInfo::RecordReturnType(FunctionBody* functionBody, ProfileId callSiteId, Var object)
{
...
ValueType
DynamicProfileInfo::GetReturnType(FunctionBody* functionBody, Js::OpCode opcode, ProfileId callSiteId) const
{
...
__inline
void
DynamicProfileInfo::RecordThisInfo(Var object, ThisType thisType)
{
...
ThisInfo
DynamicProfileInfo::GetThisInfo() const
{
    return this->thisInfo;
}

void
DynamicProfileInfo::RecordLoopImplicitCallFlags(FunctionBody* functionBody, uint loopNum, ImplicitCallFlags flags)
{
...
ImplicitCallFlags
DynamicProfileInfo::GetLoopImplicitCallFlags(FunctionBody* functionBody, uint loopNum) const
{
    Assert(Js::DynamicProfileInfo::EnableImplicitCallFlags(functionBody));
    Assert(loopNum < functionBody->GetLoopCount());
...
void
DynamicProfileInfo::RecordImplicitCallFlags(ImplicitCallFlags flags)
{
    this->implicitCallFlags = (ImplicitCallFlags)(this->implicitCallFlags | flags);
}

ImplicitCallFlags
DynamicProfileInfo::GetImplicitCallFlags() const
{
    // Mask out the dispose implicit call.  We would bailout on reentrant dispose,
    // but it shouldn't affect optimization
...
void
DynamicProfileInfo::Save(ScriptContext * scriptContext)
{
...
bool
DynamicProfileInfo::MatchFunctionBody(FunctionBody * functionBody)
{
...
FldInfo *
DynamicProfileInfo::GetFldInfo(FunctionBody* functionBody, uint fieldAccessId) const
{
    Assert(fieldAccessId < functionBody->GetProfiledFldCount());
...
ValueType
DynamicProfileInfo::GetSlotLoad(FunctionBody* functionBody, ProfileId slotLoadId) const
{
    Assert(slotLoadId < functionBody->GetProfiledSlotCount());
...
#if DBG_DUMP
void
DynamicProfileInfo::DumpProfiledValue(wchar_t const * name, CallSiteInfo * callSiteInfo, uint count)
{
    if (count != 0)
    {
...
void 
DynamicProfileInfo::DumpProfiledValue(wchar_t const * name, ArrayCallSiteInfo * arrayCallSiteInfo, uint count)
{
    if (count != 0)
    {
        Output::Print(L"    %-16s(%2d):", name, count);
...
void
DynamicProfileInfo::DumpProfiledValue(wchar_t const * name, ValueType * value, uint count)
{
    if (count != 0)
    {
        Output::Print(L"    %-16s(%2d):", name, count);
...
void
DynamicProfileInfo::DumpProfiledValue(wchar_t const * name, uint * value, uint count)
{
    if (count != 0)
    {
        Output::Print(L"    %-16s(%2d):", name, count);
...
wchar_t const *
DynamicProfileInfo::GetImplicitCallFlagsString(ImplicitCallFlags flags)
{
    // Mask out the dispose implicit call.  We would bailout on reentrant dispose,
    // but it shouldn't affect optimization
...
void
DynamicProfileInfo::DumpProfiledValue(wchar_t const * name, ImplicitCallFlags * loopImplicitCallFlags, uint count)
{
    if (count != 0)
    {
        Output::Print(L"    %-16s(%2d):", name, count);
...
void
DynamicProfileInfo::DumpProfiledValuesGroupedByValue(
...
void 
DynamicProfileInfo::DumpFldInfoFlags(wchar_t const * name, FldInfo * fldInfo, uint count, FldInfoFlags value, wchar_t const * valueName)
{
    bool header = true;
    uint lastTempFld = (uint)-1;
    for (uint i = 0; i < count; i++)
...
void
DynamicProfileInfo::DumpLoopInfo(FunctionBody *fbody)
{
...
void DynamicProfileInfo::Dump(FunctionBody* functionBody, ArenaAllocator * dynamicProfileInfoAllocator)
{
    ...
    if(Configuration::Global.flags.Verbose)
    {
        ...
        DumpProfiledValue(L"Swith opt type", this->switchTypeInfo, functionBody->GetProfiledSwitchCount());
...
void
DynamicProfileInfo::DumpList(SListBase<DynamicProfileInfo *> * profileInfoList, ArenaAllocator * dynamicProfileInfoAllocator)    
{
...
void
DynamicProfileInfo::DumpScriptContext(ScriptContext * scriptContext)
{
    if (Configuration::Global.flags.Dump.IsEnabled(DynamicProfilePhase))
    {
...
template <typename T>
bool
DynamicProfileInfo::Serialize(T * writer)
{
#if DBG_DUMP
    writer->Log(this);
#endif
...
template <typename T>
DynamicProfileInfo *
DynamicProfileInfo::Deserialize(T * reader, Recycler* recycler, Js::LocalFunctionId * functionId)
{
...
void
DynamicProfileInfo::UpdateSourceDynamicProfileManagers(ScriptContext * scriptContext)
{
    // We don't clear old dynamic data here, because if a function is inlined,it will never go thru the
    ...
    // That means that the data will never go away, probably not a good policy if this is cached for web page
    // in WININET
...
template <typename T>
void
DynamicProfileInfo::WriteData(T data, FILE * file)
{
...
template <>
void
DynamicProfileInfo::WriteData<wchar_t const *>(wchar_t const * sz, FILE * file)
{
    if (sz)
    {
...
template <typename T>
void
DynamicProfileInfo::WriteArray(uint count, T * arr, FILE * file)
{
...
template <>
void
DynamicProfileInfo::WriteData<FunctionBody *>(FunctionBody * functionBody, FILE * file)
{
