...
#undef DEF_OP
    ...
    inline bool IsExtendedOpCode(Js::OpCodeAsmJs op)    { return !IsByteOpCode(op) && (uint)op < 2 * ((uint)Js::OpCodeAsmJs::MaxByteSizedOpcodes + 1); }        /* op == [0 - 2 * 256) ?  */
    inline bool IsDblExtendedOpCode(Js::OpCodeAsmJs op) { return !IsExtendedOpCode(op)  && (uint)op < (3 * (uint)Js::OpCodeAsmJs::MaxByteSizedOpcodes + 1); }   /* op == [0 - 2 * 256) ? */
    ///----------------------------------------------------------------------------
    ///
