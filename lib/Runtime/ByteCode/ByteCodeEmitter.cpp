...
void ByteCodeGenerator::InitScopeSlotArray(FuncInfo * funcInfo)
{
    ...
    if (scopeSlotCount)
    {
        ...
        bool isSplitScope = funcInfo->paramScope && !funcInfo->paramScope->GetCanMergeWithBodyScope();

        if (funcInfo->thisScopeSlot != Js::Constants::NoRegister && !isSplitScope)
        {
            setPropertyIdForScopeSlotArray(funcInfo->thisScopeSlot, Js::PropertyIds::_lexicalThisSlotSymbol);
        }

        if (funcInfo->newTargetScopeSlot != Js::Constants::NoRegister && !isSplitScope)
        {
            setPropertyIdForScopeSlotArray(funcInfo->newTargetScopeSlot, Js::PropertyIds::_lexicalNewTargetSymbol);
        }

        if (funcInfo->superScopeSlot != Js::Constants::NoRegister && !isSplitScope)
        {
            setPropertyIdForScopeSlotArray(funcInfo->superScopeSlot, Js::PropertyIds::_superReferenceSymbol);
        }

        if (funcInfo->superCtorScopeSlot != Js::Constants::NoRegister && !isSplitScope)
        {
