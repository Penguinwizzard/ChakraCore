...
namespace Js
{
    ...
    DynamicType * PathTypeHandler::EvolveToMatchTypeHandler(PathTypeHandler *const typeHandler, DynamicObject *const object)
    {
        // Evolve the current type handler with another type handler's path and put the resulting type on the object.
        // Note that we're not checking here that the properties in the type path are not already on the current
        // handler's path.
        const bool couldSeeProto = typeHandler->GetIsOrMayBecomeShared() && !typeHandler->IsolatePrototypes();
        PathTypeTransitionInfo *currentTransitionInfo = GetTransitionInfo();
        PropertyIndex slotIndex = 0;
        TypePath *const typePath = typeHandler->GetTypePath();
        const PropertyIndex slotCount = typeHandler->GetSlotCountInternal();
        ...
        DynamicType *const type = 
            GetOrCreateType(currentTransitionInfo, slotCount, typeHandler->GetPropertyCountInternal(), true, nullptr);
        object->ReplaceType(type);
        return type;
    }
    ...
    void PathTypeHandler::ChangeAllSlotStorageToVar(DynamicObject *const object)
    {
            ...
            oldSlotIndex = typePath->GetNextSlotIndex(oldSlotIndex), ++newSlotIndex)
        {
            ...
            if((!ObjectSlotType::RequiresWideSlotSupport() || newSlotIndex == oldSlotIndex) && 
               oldSlotType.IsVar() &&
               (oldSlotIndex < this->GetInlineSlotCapacity()) == (newSlotIndex < newTypeHandler->GetInlineSlotCapacity()))
            {
                continue;
            }
