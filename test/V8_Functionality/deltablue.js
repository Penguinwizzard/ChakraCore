...
Constraint.prototype.satisfy = function (mark) {
  ...
  if (!this.isSatisfied()) {
    if (this.strength == Strength.REQUIRED)
      WScript.Echo("Could not satisfy a required constraint!");
    return null;
  }
  ...
  if (!planner.addPropagate(this, mark))
    WScript.Echo("Cycle encountered");
  out.mark = mark;
  return overridden;
}
...
function chainTest(n) {
  ...
  for (var i = 0; i < 100; i++) {
    first.value = i;
    plan.execute();
    WScript.Echo(last.value);
    if (last.value != i)
      WScript.Echo("Chain test failed.");
  }
}
...
function projectionTest(n) {
  ...
  change(src, 17);
  WScript.Echo(dst.value);
  if (dst.value != 1170) WScript.Echo("Projection 1 failed");
  change(dst, 1050);
  WScript.Echo(src.value);
  if (src.value != 5) WScript.Echo("Projection 2 failed");
  change(scale, 5);
  for (var i = 0; i < n - 1; i++) {
    if (dests.at(i).value != i * 5 + 1000)
      WScript.Echo("Projection 3 failed");
  }
  change(offset, 2000);
  for (var i = 0; i < n - 1; i++) {
    if (dests.at(i).value != i * 5 + 2000)
      WScript.Echo("Projection 4 failed");
  }
}
...
//WScript.Echo("DeltaBlue: " + elapsed + 'ms');