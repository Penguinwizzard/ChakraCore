...
"@ -f $BuildType, $BuildPlatform, $BuildConfiguration)
...
$BinariesDirectory = UseValueOrDefault $Env:BUILD_BINARIESDIRECTORY (Join-Path $SourcesDirectory "Build\VcBuild")
# Note: as of now there is no use of the $Env:BUILD_OBJECTDIRECTORY variable from VSTS or otherwise but it is here for symmetry with the similar environment variables
$ObjectDirectory = UseValueOrDefault $Env:BUILD_OBJECTDIRECTORY (Join-Path $BinariesDirectory "obj\${BuildPlatform}_${BuildConfiguration}")

$VersionMajor = UseValueOrDefault $verMajor $Env:VERSION_MAJOR "0"
$VersionMinor = UseValueOrDefault $verMinor $Env:VERSION_MINOR "0"
...
$VersionString = "${VersionMajor}.${VersionMinor}.${buildPushIdString}"
$BranchNameSemVerSanitized = $BranchName -replace ("[^a-zA-Z\d-]", "-")
$PreviewVersionString = "${VersionString}-preview.${BranchNameSemVerSanitized}+$CommitHash"
